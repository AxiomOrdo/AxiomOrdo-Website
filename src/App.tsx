...
const brands: Brand[] = [
  ...
  // ── 9. Meriden Compliance ── Maritime SME ────────────────────────────────
  {
    ...
    tagline: "Practical maritime resources and articles.",
    description:
      "Maritime resources and articles published by Meriden Compliance, an AxiomOrdo Ltd brand.",
    color: "#1d4ed8",
    accent: "#60a5fa",
...
function MeridenDestination() {
  const { pathname, search, hash } = useLocation();
  const articlePath = pathname.replace(/^\/meriden-compliance/, "");
  const category = articlePath.split("/")[2];
  const destination = articlePath.startsWith("/insights")
    ? (["maritime-ai", "maritime-qhse"].includes(category) ? articlePath : "/insights")
    : "/";
  const url = `https://www.meridencompliance.com${destination}${search}${hash}`;
  useEffect(() => { window.location.replace(url); }, [url]);
  return <main><p>Meriden Compliance has its own website.</p><a href={url}>Continue to Meriden Compliance</a></main>;
}
  ...
  return (
    <BrowserRouter>
      ...
      <Routes>
        ...
        {/* Meriden publishes products and articles in its own repository/site. */}
        <Route path="/meriden-compliance/*" element={<MeridenDestination />} />
        <Route path="/insights/*" element={<MeridenDestination />} />
        <Route path="/authors/phillip-inzaghi" element={<PhillipAuthorPage />} />

        {/* Brand sub-sites */}
        {brands.map((brand) =>
          brand.key === "meriden" ? (
            <Route key={brand.key} path={brand.href} element={<MeridenDestination />} />
          ) : unreleasedBrandKeys.has(brand.key) ? (
